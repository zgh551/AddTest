...
LINKFLAGS='-z -m"%s/%s.map" --heap_size=0x200 --stack_size=0x200 --warn_sections -i"%s" -i"%s" --reread_libs --diag_wrap=off --display_error_number --xml_link_info="%s/%s_linkInfo.xml" --rom_model ' %(BUILD_INC,ProjName,C2000_LIB,C2000_INC,BUILD_INC,ProjName),
